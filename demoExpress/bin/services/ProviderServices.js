...
exports.delete = async (id) => {
    // delete from prodiver where id = id
    const removeById = (providers, id) => {
        const requiredIndex = providers.findIndex(el => {
            return el.id == id;
        });
        if (requiredIndex === -1) {
            return false;
        };
        return !!providers.splice(requiredIndex, 1);
    };
    removeById(providers, id);
}
